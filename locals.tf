locals{
    ...
    host_header = var.component == "frontend" ? ["${var.component}-${local.project}-${local.environment}.${var.domain_name}"] : ["${var.component}.${local.project}-${local.environment}-backend-alb.${var.domain_name}"]
}